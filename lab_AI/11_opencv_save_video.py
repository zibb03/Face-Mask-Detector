...
while cap.isOpened():
    ...
    for i in range(face_locations.shape[2]):
        ...
        left = int(face_locations[0, 0, i, 3] * width) # 비율 값을 가지고 있어서 width, height 값을 곱해주어야 함
        top = int(face_locations[0, 0, i, 4] * height)
        right = int(face_locations[0, 0, i, 5] * width)
        bottom = int(face_locations[0, 0, i, 6] * height)

        face_image = image[top:bottom, left:right] # top에서 bottom까지, left에서 right까지 잘라주는 코드 (파이썬의 장점)
